...
To display an in-app message, Iterable's Web SDK uses an `iframe` on which the `sandbox` attribute is set to `allow-same-origin allow-popups allow-top-navigation`.  On Safari, this configuration blocks JavaScript execution in tabs that open as the result of clicks on links included in the `iframe`.
...
For more information about `allow-popups-to-escape-sandbox`, see:
...
- [Can I Use](https://caniuse.com/mdn-html_elements_iframe_sandbox_allow-popups-to-escape-sandbox)

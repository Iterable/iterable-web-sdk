...
export function initialize(
  ...
  */
  ...
  const handleMerge = async (
    ...
    if (sourceUserId === null && sourceEmail === null) {
      // promise resolves here because merging is not needed here
      // It is a scenario where criteria was not met so there sourceID is null
    ...
    return {
      ...
      setEmail: async (email: string, merge?: boolean) => {
        ...
        try {
          ...
          const result = await handleMerge(email, true, merge);
          if (result) {
      ...
      setUserID: async (userId: string, merge?: boolean) => {
        ...
        try {
          merge = getMergeDefaultValue(merge);
          const result = await handleMerge(userId, false, merge);
          if (result) {
      ...
      logout: () => {
        anonUserManager.removeAnonSessionNCriteriaData();
        typeOfAuth = null;
        authIdentifier = null;

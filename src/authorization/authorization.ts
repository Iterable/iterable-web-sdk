...
export function initialize(
  ...
  */
  ...
  const getMergeDefault = (merge?: boolean) => {
    console.log('authIdentifier', authIdentifier);
    console.log('typeOfAuth', typeOfAuth);
    console.log('anon user', getAnonUserId());

    const doesAnonUserExist =
      getAnonUserId() === null || getAnonUserId() === undefined;
    return merge === undefined
      ? (authIdentifier === null && typeOfAuth === null && doesAnonUserExist) || // Criteria is not yet met (default merge is true)
        (authIdentifier !== null && typeOfAuth !== null && !doesAnonUserExist) // Criteria is met (Iterable profile created with an autogenerated identity)(default merge is true)
        ? true
        : false // Current logged in user is identified (default merge is false)
      : merge;
  };

  const getSourceDestinationEmailOrUser = async (
  ...
  const tryMergeUser = async (
    ...
    console.log('auth identifier and merge', authIdentifier, merge);
  ...
  if (!generateJWT) {
    ...
    return {
      ...
      setEmail: async (email: string, merge?: boolean) => {
        clearMessages();
        try {
          merge = getMergeDefault(merge);
          const result = await getSourceDestinationEmailOrUser(
            email,
            true,
            merge
          );
      ...
      logout: () => {
        anonUserManager.removeLocaStorageData();
        typeOfAuth = null;
        authIdentifier = null;

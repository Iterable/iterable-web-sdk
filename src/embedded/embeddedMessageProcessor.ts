...
export class EmbeddedMessagingProcessor {
    ...
    constructor(currentMessages: Array<IEmbeddedMessage>, fetchedMessages: Array<IEmbeddedMessage>) {
    ...
    public getNewMessages() {
        return this.fetchedMessages.filter((messageInfo: IEmbeddedMessage) => !this.getCurrentMessageIds().includes(messageInfo.metadata.messageId))
    }
    
    public getCurrentMessageIds(): string[] {
        return this.currentMessages.map((messageInfo: IEmbeddedMessage) => messageInfo.metadata.messageId)
    }
    
    public getFetchedMessageIds(): string[] {
        return this.fetchedMessages.map((messageInfo: IEmbeddedMessage) => messageInfo.metadata.messageId)
    }
}
...
class EmbeddedSession {
    ...
    constructor(start?: Date, end?: Date, placementId?: string, impressions?: Array<IEmbeddedImpression>) {
        this.start = start;
        this.end = end;
...
export class EmbeddedSessionManager {
    ...
    public startImpression(messageId: string) {
        var impressionData: IEmbeddedImpressionData | undefined = this.impressions.get(messageId)
    ...
    public pauseImpression(messageId: string) {
        const impressionData: IEmbeddedImpressionData | undefined = this.impressions.get(messageId);
    ...
    private getImpressionList() {
        ...
        this.impressions.forEach((impressionData) => {
            impressionList.push(new EmbeddedImpressionData(
    ...
    private updateDisplayCountAndDuration(impressionData: IEmbeddedImpressionData) {
        if (impressionData.start) {
            impressionData.displayCount = impressionData.displayCount + 1;
            impressionData.duration = impressionData.duration + ((new Date().getTime() - impressionData.start.getTime()) / 1000.0)

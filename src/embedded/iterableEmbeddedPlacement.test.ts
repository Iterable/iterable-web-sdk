...
describe('EmbeddedManager', () => {
  test('embeddedPlacementDeserialization_elementsAndCustomPayloadDefined', () => {
    ...
    if (jsonArray !== null) {
      ...
      const btn = message.elements?.buttons?.length
      ...
      const txt = message.elements?.text?.length
        ? message.elements?.text[0]
        : null;
  ...
  test('embeddedPlacementSerialization_elementsAndCustomPayloadDefined', () => {
    ...
    const embeddedMessageDefaultAction =
      new EmbeddedMessageElementsDefaultAction('someType', 'someData');

    const embeddedMessageElementsButtonAction =
      new EmbeddedMessageElementsButtonAction(
        'openUrl',
        'https://www.google.com'
      );

    const embeddedMessageButtons = [
      new EmbeddedMessageElementsButton(
        'reward-button',
        'REDEEM MEOW',
        embeddedMessageElementsButtonAction
      )
    ...
    const embeddedMessagePlacement = new IterableEmbeddedPlacement(
      placementId,
      messages
    );
    ...
    if (jsonArray !== null) {
      ...
      const placementJson = IterableEmbeddedPlacement.toJSONObject(
        embeddedMessagePlacement
      );
  ...
  test('embeddedPlacementSerialization_noButtons_noText', () => {
    ...
    if (jsonArray !== null) {
      ...
      expect(placementJson.embeddedMessages[0]).toEqual(expectedPlacementJson);
    }
  });
});
